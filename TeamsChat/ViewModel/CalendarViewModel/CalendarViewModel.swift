...
extension CalendarViewModel {
    ...
    func requestAccessToCalendar(isTestData: Bool = false, status: EKAuthorizationStatus = .authorized, completion: @escaping ((Bool) -> Void)) {
        
        let ekAuthorizationStatus: EKAuthorizationStatus = isTestData ? status : EKEventStore.authorizationStatus(for: .event)
        switch  ekAuthorizationStatus {
        case .authorized:
            completion(true)
